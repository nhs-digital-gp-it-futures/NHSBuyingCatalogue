...
namespace NHSD.GPITF.BuyingCatalog.Controllers
{
  ...
  public sealed class FrameworkController : Controller
  {
    ...
    public FrameworkController(IFrameworkLogic logic)
    {
      _logic = logic;
    }
    ...
    [ValidateModelState]
    [SwaggerResponse(statusCode: (int)HttpStatusCode.OK, type: typeof(PaginatedList<Framework>), description: "Success")]
    ...
    public IActionResult ByCapability([FromRoute][Required]string capabilityId, [FromQuery]int? pageIndex, [FromQuery]int? pageSize)
    {
      ...
      var retval = PaginatedList<Framework>.Create(frameworks, pageIndex, pageSize);
    ...
    [Route("ById/{id}")]
    [ValidateModelState]
    [SwaggerResponse(statusCode: (int)HttpStatusCode.OK, type: typeof(Framework), description: "Success")]
    ...
    [SwaggerRequestExample(typeof(Framework), typeof(FrameworkExample), jsonConverter: typeof(StringEnumConverter))]
    ...
    [HttpGet]
    [ValidateModelState]
    [SwaggerResponse(statusCode: 200, type: typeof(PaginatedList<Framework>), description: "Success - if no frameworks found, return empty list")]
    public IActionResult Get([FromQuery]int? pageIndex, [FromQuery]int? pageSize)
    {
      var allFrameworks = _logic.GetAll();
      var retval = PaginatedList<Framework>.Create(allFrameworks, pageIndex, pageSize);
    ...
    [SwaggerRequestExample(typeof(Framework), typeof(FrameworkExample), jsonConverter: typeof(StringEnumConverter))]
    public IActionResult Update([FromBody]Framework framework)
    {
      try
      {
