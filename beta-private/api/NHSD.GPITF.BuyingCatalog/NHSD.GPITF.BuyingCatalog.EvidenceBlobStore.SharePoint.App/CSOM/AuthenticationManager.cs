...
namespace NHSD.GPITF.BuyingCatalog.EvidenceBlobStore.SharePoint.App.CSOM
{
  /// <summary>
  ...
  public sealed class AuthenticationManager : IAuthenticationManager
  {
      ...
      string globalEndPointPrefix)
    {
      if (_appOnlyAccessToken == null)
      {
        lock (_tokenLock)
        {
          _logger.LogDebug("AuthenticationManager:EnsureToken --> (siteUrl:{0},realm:{1},appId:{2},appSecret:{3})", siteUrl, realm, appId, appSecret);
          if (_appOnlyAccessToken == null)
          {
