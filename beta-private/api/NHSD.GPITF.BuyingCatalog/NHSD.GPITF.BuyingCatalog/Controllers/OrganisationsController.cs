...
namespace NHSD.GPITF.BuyingCatalog.Controllers
{
  ...
  public sealed class OrganisationsController : Controller
  {
    ...
    [HttpGet]
    [ValidateModelState]
    [SwaggerResponse(statusCode: (int)HttpStatusCode.OK, type: typeof(PaginatedList<Organisations>), description: "Success - if no Organisation found, return empty list")]
    public IActionResult Get([FromQuery]int? pageIndex, [FromQuery]int? pageSize)
    {
      var allOrgs = _logic.GetAll();
      var retval = PaginatedList<Organisations>.Create(allOrgs, pageIndex, pageSize);
    ...
    [ValidateModelState]
    [SwaggerResponse(statusCode: (int)HttpStatusCode.OK, type: typeof(Organisations), description: "Success")]
    public IActionResult Create([FromBody]Organisations org)
    {
    ...
    public IActionResult Update([FromBody]Organisations org)
    {
      try
      {
        _logic.Update(org);
    ...
    [SwaggerResponse(statusCode: (int)HttpStatusCode.OK, description: "Success")]
    [SwaggerResponse(statusCode: (int)HttpStatusCode.NotFound, description: "Organisation not found in CRM")]
    public IActionResult Delete([FromBody]Organisations org)
    {
      try
      {
        _logic.Delete(org);
