...
docker run --rm -d -it \
  ...
  -v betaprivate_db_data:/var/opt/mssql \
...
docker exec -it setup_sql_server /bin/bash -c "cd opt/data; echo 'Waiting for SQL Server...'; ./wait-for-it.sh localhost:1433 -- ./FromScratch.MSSQLServer.sh"
